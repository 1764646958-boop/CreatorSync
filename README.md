...
| DeepSeek 改写 | 已实现 | 配置 `DEEPSEEK_API_KEY` 后通过 DeepSeek Chat Completions 生成平台文案，仍保持统一接口。 |
| mock fallback | 已实现 | 无 DeepSeek Key 或开启 `AI_MOCK_MODE=true` 时使用确定性平台规则生成结果，保证本地可复现。 |
...
flowchart LR
  ...
  AdaptAPI --> DeepSeek{DeepSeek configured?}
  DeepSeek -->|yes| LLM[DeepSeek Chat Completions]
  DeepSeek -->|no / failure| Registry[Adapter Registry]
  LLM --> Registry
...
4. 如果配置了 `DEEPSEEK_API_KEY` 且未开启 `AI_MOCK_MODE`，改写服务会调用 DeepSeek；无 Key、强制 mock 或 DeepSeek 请求失败时自动回退到 Adapter 的确定性 mock fallback。
5. Adapter Registry 按平台 ID 找到具体 Adapter，并通过统一 `adapt(input)` 方法提供 fallback seed 与平台结构。
6. 每个结果返回 `{ platform, status, content, warnings, metadata }` 结构，其中 `metadata.generationMode` 标识 `deepseek` 或 `mock_fallback`。
7. 前端用统一结果渲染平台预览；用户可继续触发 mock publish。
8. 发布历史模块从本地 `data/publish-history.json` 读取记录并支持导出；后续接入真实发布后可复用该模块记录任务结果。
...
| 改写服务 | `backend/src/services/adaptation-service.ts` | 组装 prompt 上下文、调用 DeepSeek 或回退 Adapter、返回多平台结果。 |
| DeepSeek 配置/客户端 | `backend/src/services/ai-config.ts`, `backend/src/services/deepseek-client.ts` | 读取 DeepSeek Key、Base URL / Endpoint 与模型环境变量，并调用 OpenAI-compatible Chat Completions endpoint。 |
...
- 当前已通过 Node.js 内置 `fetch` 接入 DeepSeek OpenAI-compatible Chat Completions，不新增 AI SDK 依赖；无 `DEEPSEEK_API_KEY` 时仍使用 mock fallback。
...
建议后端本地端口使用 `3001`，前端默认 `NEXT_PUBLIC_API_BASE_URL=http://localhost:3001`。如果需要改为其他端口，请同步调整后端 `PORT` 和前端 `NEXT_PUBLIC_API_BASE_URL`。要启用真实 DeepSeek 改写，请填写 `DEEPSEEK_API_KEY`；不填写或设置 `AI_MOCK_MODE=true` 会保持 mock fallback。
...
该命令会通过 `scripts/dev.mjs` 同时启动后端与前端，并把根目录 `.env` 中的 `PORT` / `NEXT_PUBLIC_API_BASE_URL` 传给子进程。
...
- 扩展更多真实 AI Provider，并保留 DeepSeek 与 mock fallback 作为评审和离线演示模式。
